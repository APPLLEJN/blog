  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$store.dispatch('POST', {
        ...
        const url = _this.$store.state.siteInfo.siteUrl.value
        const path = _this.form.pathName
        const timestamp = new Date(_this.form.updatedAt).valueOf()
        const key = `${url}|${path}`
        this.saveLS(key, _this.form.markdownContent + `|${timestamp}`)
        this.$message({
          message: '文章已成功提交',
          type: 'success'
        })
        if (response._id && this.id === -1) {
          _this.$router.replace({ params: { id: response._id } })
          _this.form = response
